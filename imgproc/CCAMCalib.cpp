...
int CCAMCalib::AddChessboardPoints(const std::vector<std::__cxx11::string> &_filelist, cv::Size &_board_size)
{
...
cv::Mat CCAMCalib::ReMap(const Mat &_image)
{
    ...
    string param_dir = home_dir + "/qt_workspace/ipmapper/imgproc/CAM_param.xml";

    FileStorage fs(param_dir,FileStorage::READ);
    fs["m_camera_matrix"] >> m_camera_matrix;
    fs["m_dist_coeffs"] >> m_dist_coeffs;
    fs.release();

    cv::Size size_remap;
    size_remap.width = _image.size().width;
    size_remap.height = _image.size().height;
